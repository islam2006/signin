...
def registration():
    ...
    def signup():
        ...
        if len(login_register.get()) == 0:
            error = '*login error'
        elif (len(password1.get()) < 7 or len(password1.get()) > 14):
            error = '*your password needs to be at least 6 and more 14 character '
...
def login_form():
    ...
    def login_pass():
        ...
        if enter_login.get() in a and enter_password.get() == a[enter_login.get()]:
            messagebox.showinfo('Welcome', 'WELCOME TO THE PIRACY GAME')
        else:
...
HEIGHT = 380
root = Tk()
root.title('Login Form')("salam. islamin %s yasi var")
...
button_signup = Button(root, text='SIGN UP', bg='blue', command=registration)
button_signup.place(relx=0.2, rely=0.1, relwidth=0.3)
button_signin = Button(root, text='SIGN IN', bg='blue', command=login_form)
